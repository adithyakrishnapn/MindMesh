...
export default function Stats() {
  return (
    <section id="stats" className="relative w-full py-20 bg-meshOffWhite border-b-4 border-meshBlack overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        
        {/* Section Header */}
        <div className="mb-12 flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            <span className="badge-sticker mb-2">HACKATHON AT A GLANCE</span>
            <h2 className="font-display font-black text-4xl sm:text-6xl text-meshBlack uppercase tracking-tight">
              BY THE <span className="text-meshYellow drop-shadow-[2px_2px_0px_#111111]">NUMBERS</span>
            </h2>
          </div>
          <p className="font-mono text-xs font-bold text-meshGray uppercase tracking-widest max-w-xs">
            HIGH ENERGY • INTENSE CODING • REAL IMPACT
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {hackathon.stats.map((stat, idx) => (
            <motion.div
              ...
              className={`card-playful p-6 flex flex-col justify-between min-h-[240px] relative overflow-hidden ${
                idx % 2 === 1 ? "bg-white" : "bg-meshYellow/20"
              }`}
            >
              {/* Background oversized unit text opacity */}
              <div className="absolute right-2 -bottom-4 font-display font-black text-7xl opacity-10 select-none text-meshBlack pointer-events-none">
                {stat.unit}
              </div>
              ...
              <div>
                ...
                <div className="font-mono font-extrabold text-sm uppercase tracking-widest text-meshBlack mt-2 bg-meshYellow px-2 py-0.5 inline-block border border-meshBlack">
                  {stat.unit}
                </div>
              </div>
              ...
              <div className="mt-6 pt-4 border-t-2 border-meshBlack">
                <h3 className="font-display font-bold text-base text-meshBlack">
                  {stat.label}
                </h3>
